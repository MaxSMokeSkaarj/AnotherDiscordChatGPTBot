    ...
    async run() {
        ...
        */
        ...
        client.on( 'messageCreate', async ( message ) => {
            ...
            {
                MessageHistory = [];
                fs.writeFileSync( './json/history.json', JSON.stringify( MessageHistory, '\n', 4 ) ); // очистка масссива и запись в фаил, следующее сообщение бота будет записано 
                message.reply( "История очищена!" );
                return; // прерываем, что-бы обработчик ChatGPT не начал обрабатывать то, что не надо
            }
            ...
            if ( message.author.id == client.user.id ) // селектор user/assitiant и добавление в память сообщения
            {
                MessageHistory.push( {
                    role: 'assistant',
            ...
            let response = await this.getChatGPTAnswer( this.ChatGPTModel, this.ChatGPTModel, MessageHistory );
            response = response.data.choices[ 0 ].message.content;
            // Отправляем ответ в Discord канал
            if ( response.length < 2000 ) message.reply( response );
            else {
                fs.writeFileSync( './message.txt', response );
                message.reply( {
                    files: [ './message.txt' ]
                } );
